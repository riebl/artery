...
void CpService::initialize()
{
	...
	mMinGroundVelocityOrientationChangeThreshold = vanetza::units::Angle { par("minGroundVelocityOrientationChangeThreshold").doubleValue() * vanetza::units::degree };
...
void CpService::capturePerceivedObjectSnapshot(const omnetpp::SimTime& T_now, uint64_t referenceTime, const LocalEnvironmentModel::TrackedObjects& objects, const ObjectVdpSnapshotMap& objectVdpSnapshots)
{
...
bool CpService::checkPerceivedObjectTrigger(const SimTime& T_now)
{
	...
	for (const auto idx : mSelectedCpmObjects) {
		...
		// objects with prior inclusion history already carry cached delta metrics, while objects without history (newly detected) have zeros in the delta metrics
		...
		mPerceivedObjectHistories[static_cast<int>(po.cpsId)] = PerceivedObjectHistory{
			T_now,
			Position(static_cast<double>(po.xCm), static_cast<double>(po.yCm)),
			(po.hasVelocity ? po.speedMps : 0.0),
...
double CpService::calculateUtilityFunction(const PerceivedObjectSnapshot& po, double distanceDiff, double speedDiff, double orientationDiff, double lastInclusionSeconds)
{
	...
	if (speedDiff > mMinGroundSpeedChangePriorityThreshold.value()) {
		ouf += std::min(1.0, (speedDiff - mMinGroundSpeedChangePriorityThreshold.value()) / (mMaxGroundSpeedChangePriorityThreshold.value() - mMinGroundSpeedChangePriorityThreshold.value()));
	}
	if (orientationDiff > mMinGroundVelocityOrientationChangePriorityThreshold.value()) {
		ouf += std::min(1.0, (orientationDiff - mMinGroundVelocityOrientationChangePriorityThreshold.value()) / (mMaxGroundVelocityOrientationChangePriorityThreshold.value() - mMinGroundVelocityOrientationChangePriorityThreshold.value()));
	}
	if (lastInclusionSeconds > mMinLastInclusionTimePriorityThreshold.dbl()) {
		ouf += std::min(1.0, (lastInclusionSeconds - mMinLastInclusionTimePriorityThreshold.dbl()) / (mMaxLastInclusionTimePriorityThreshold.dbl() - mMinLastInclusionTimePriorityThreshold.dbl()));
	}
...
void addPerceivedObjectContainer(Cpm& message, const std::vector<CpService::PerceivedObjectSnapshot>& perceivedObjectSnapshot, const std::vector<std::size_t>& selectedObjects, const omnetpp::SimTime& lastCpmTimestamp)
{
	Vanetza_ITS2_WrappedCpmContainer_t* wcc = vanetza::asn1::allocate<Vanetza_ITS2_WrappedCpmContainer_t>();
